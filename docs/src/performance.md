...
Virtual machines and GitHub-hosted runners are useful for release-to-release comparisons, but the numbers should not be treated as absolute hardware limits. Small deltas under 5% are treated as normal runner/runtime variation, not real regressions. When new generators are added, aggregate rows become `workload changed`; use matching per-generator rows for regression checks.
